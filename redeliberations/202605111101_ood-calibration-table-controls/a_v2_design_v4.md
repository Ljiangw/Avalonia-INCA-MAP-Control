...
5. **修正 `DataGrid` 虚拟化联动机制**：删除对 Avalonia 不存在的 `UnloadingRow` 事件的依赖，改用 `LoadingRow` 先清理再判断的策略，并补充 `DataContextChanged` 作为后备清理机制。
6. **补全 `readonly struct` 行数据的外部数据变更刷新路径**：为 `ITableDataSource<TRow>` 补充 `Refresh()` 方法，明确非编辑路径下的数据变更由 Editor 主动触发全量刷新。
...
- `void LoadMapData(IMapData data)` — 加载二维 Map 数据（MapEditor 使用）
...
用户触发单元格进入编辑模式（双击）→ 单元格展示编辑模板（如 `TextBox`）→ 用户输入数值 → 输入完成时（Enter/失去焦点），Editor 拦截 `CellEditEnding` 事件，从对应类型的行数据对象中提取坐标索引属性，调用原始数据模型的写方法更新对应位置 → 模型触发 `INotifyPropertyChanged.PropertyChanged` → Editor 订阅到该通知后调用 `ITableDataSource<TRow>.Refresh()` 重新生成行集合，并重新设置 `DataGrid.ItemsSource`，完成表格刷新；若该单元格值同时影响图表，图表呈现器接收通知后重绘。
