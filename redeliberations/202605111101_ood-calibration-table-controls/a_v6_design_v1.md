# Avalonia 标定数据表格编辑控件 — 架构级 OOD 设计方案 v8
...
相对于 v7 方案，v8 的核心变化集中在以下方面：

1. **修复 Avalonia DataGrid `CurrentCell` 事实错误**：将文档中所有 `DataGrid.CurrentCell` 引用统一替换为 Avalonia 实际 API 组合（`SelectedItem` + `CurrentColumn`/`DisplayIndex` + `BeginEdit()`），覆盖编辑回写、键盘导航、z 切片激活联动等全部关键路径，消除 WPF 概念残留导致的编译阻塞风险。
2. **降级脏标记为全局未保存提示**：移除「单元格级脏标记（橙色边框）」承诺，明确 `IAsyncSaveCapable` 仅支撑全局未保存状态提示；控件层不维护逐单元格脏状态。
3. **修复 `ActiveZSliceChangedEventArgs` 数组越界风险**：规定 `OldZIndex`/`NewZIndex < 0` 时对应 `OldZValue`/`NewZValue` 返回 `double.NaN`；`ActiveSliceChanged` 事件仅在有效索引间切换时触发，杜绝 `ZValues[-1]` 运行时异常。
4. **补充 `ILineChartPresenter.SetAxisLabels` 契约**：增加 `void SetAxisLabels(string xName, string xUnit, string zName, string zUnit)` 方法，与 `ISurfaceChartPresenter` 保持对称，确保折线图轴标签功能可验证。
5. **明确全量/增量刷新后的选中状态恢复策略**：全量刷新（`Refresh()` + 重新设置 `ItemsSource`）后只能使用「行索引 + 列索引」恢复选中；增量刷新（`ReplaceRow`）下因不替换 `ItemsSource`，可保留原有行对象引用恢复策略。
6. **在 `ITableDataSource<TRow>` 中明确排序契约**：`Rows` 集合必须按原始数据多维坐标顺序排列——`CubeRowData` 按 `ZIndex` 升序分组、组内按 `YIndex` 升序排列；`MapRowData` 按 `YIndex` 升序排列。排序是适配器的强制职责。
7. **明确 `MapRowData`/`CubeRowData` 行头值为 `string` 类型**：`ZValue`/`YValue` 由 `CubeTableAdapter`/`MapTableAdapter` 在生成行数据时按精度策略（`FormatString` → `DisplayPrecision` → `G6`）预先格式化；行头列的 `CellTemplate` 直接绑定纯文本展示，无需在模板层重复处理精度逻辑。
8. **在 `ITableDataSource<TRow>` 中新增精度信息暴露点**：增加 `string? FormatString { get; }` 和 `int DisplayPrecision { get; }` 属性，支撑 `MapEditor`/`CubeEditor` 动态生成数据列时设置 `Binding.StringFormat`。
9. **修正行级高亮驱动源**：将驱动源从不可外部访问的 `DataGrid.CurrentItem`（protected）修正为 `DataGrid.SelectedItem`（public），通过 `SelectionChanged` 事件或 `SelectedItemProperty` 变更实现行级高亮。
10. **补充 `CubeEditor` z 列点击边界条件**：在「z 切片激活与 3D 视图联动」中补充 `XAxisLength == 0` 时的防护逻辑——此时不存在数据单元格，跳过"选中第一个数据单元格"步骤。
11. **补充图表呈现器生命周期管理**：在三个 Editor 的职责描述及变量切换流程中明确 `IAvaloniaChartPresenter.Detach()` 的调用时机（变量切换导致旧图表丢弃时、控件从视觉树卸载时），防止 GPU/渲染资源泄露。
...
**关键成员**：
- `bool HasUnsavedChanges { get; }` — 是否存在尚未持久化的变更；控件层据此决定是否显示全局未保存状态提示
...
- 全局未保存状态提示：当数据模型实现 `IAsyncSaveCapable` 时，订阅其 `UnsavedChangesChanged` 事件，在信息栏区域显示未保存数量（如 "3 处未保存"），数据源为 `IAsyncSaveCapable.UnsavedChangeCount`。若 `UnsavedChangeCount` 只能提供布尔降级（返回 0 或 1），则显示 "存在未保存变更"。**控件层不维护逐单元格脏状态**，仅提供全局级别的未保存提示
...
**生命周期管理**：三个 Editor 在以下场景必须调用内部持有的 `IAvaloniaChartPresenter`（或其维度特化子接口）实例的 `Detach()` 方法，释放图表渲染目标与 GPU/渲染资源：
- **变量切换时**：旧变量对应的图表数据被丢弃前，先调用旧呈现器的 `Detach()`
- **控件从视觉树卸载时**：在 `OnDetachedFromVisualTree` 覆盖中调用当前呈现器的 `Detach()`
- **控件销毁时**：确保 `Detach()` 在呈现器实例被 GC 之前执行

未调用 `Detach()` 可能导致 GPU 上下文、渲染目标纹理或后台渲染线程泄露。
...
**排序契约**：`Rows` 集合必须按原始数据的多维坐标顺序排列：
- `CubeRowData` 按 `ZIndex` 升序分组，组内按 `YIndex` 升序排列
- `MapRowData` 按 `YIndex` 升序排列

排序是适配器的强制职责，不可省略。`ZSliceActivationTracker` 和 z 切片分组线均隐含「相同 z 值的行在 `Rows` 集合中连续出现」的假设，若未排序将导致 z 切片级高亮的视觉呈现严重受损。
...
- `string? FormatString { get; }` — 数值格式字符串（如 `F3`、`G6`），由适配器从 `ICalibrationData.FormatString` 透传；供 Editor 动态生成 `DataGrid` 数据列时设置 `Binding.StringFormat`
- `int DisplayPrecision { get; }` — 默认显示精度（小数位数），由适配器从 `ICalibrationData.DisplayPrecision` 透传；供 Editor 在 `FormatString` 为空时构造 `F{displayPrecision}` 格式的 `StringFormat`
...
**职责**：
- `MapRowData`：内联 `YIndex` 属性（`int` 类型），标识该行在二维矩阵中的 y 坐标；承载 `YValue` 属性（`string` 类型，由 `MapTableAdapter` 按精度策略预先格式化后的 y 轴行头展示文本）。x 列数据值通过 `IReadOnlyList<double> Values { get; }` 按列顺序暴露
- `CubeRowData`：内联 `ZIndex`、`YIndex` 属性（均为 `int` 类型），标识该数据行在三维张量中的坐标；承载 `ZValue` 和 `YValue` 属性（均为 `string` 类型，由 `CubeTableAdapter` 按精度策略预先格式化后的 z/y 轴行头展示文本）。x 列数据值通过 `IReadOnlyList<double> Values { get; }` 按列顺序暴露，列索引与 x 轴索引一一映射
...
- `void SetAxisLabels(string xName, string xUnit, string zName, string zUnit)` — 设置折线图各轴的名称和单位。x 轴为水平坐标轴，z 轴为垂直坐标轴。在 `LoadData` 之后调用，确保图表在展示数据的同时正确标注轴信息
...
- `event EventHandler<ActiveZSliceChangedEventArgs> ActiveSliceChanged` — 激活切片变更事件。参数携带旧 `ZIndex` 和新 `ZIndex`。事件仅在防抖计时结束后（用户停止快速导航）**且 `OldZIndex >= 0` 与 `NewZIndex >= 0` 同时成立**时触发，避免无效索引导致数组越界，同时避免频繁通知 3D 图表重绘
...
- `double OldZValue { get; }` — 变更前的 z 轴实际数值。当 `OldZIndex >= 0` 时由 `ICubeData.ZValues[OldZIndex]` 提供；当 `OldZIndex < 0` 时返回 `double.NaN`
- `double NewZValue { get; }` — 变更后的 z 轴实际数值。当 `NewZIndex >= 0` 时由 `ICubeData.ZValues[NewZIndex]` 提供；当 `NewZIndex < 0` 时返回 `double.NaN`
...
[CalibrationEditorBase<T> 更新 SelectedVariable]
    ...
    └── ItemsSource 非空，SelectedVariable 有效，且数据维度均大于零
        └── [触发 OnExitEmptyState] ──→ [触发 OnSelectedVariableChanged]
                                        │
                                        ├── CurveEditor: 通知 ICurveTablePresenter 重建，调用旧 ILineChartPresenter.Detach() 后通知新 ILineChartPresenter 加载并调用 `SetAxisLabels` 设置轴标签
                                        ├── MapEditor: 重建 ITableDataSource，调用旧 ISurfaceChartPresenter.Detach() 后通知新 ISurfaceChartPresenter 加载
                                        └── CubeEditor: 重建 ITableDataSource，调用 ZSliceActivationTracker.ResetToDefault()，调用旧 ISurfaceChartPresenter.Detach() 后通知新 ISurfaceChartPresenter 加载
```
...
2. **验证通过**：
   ...
   - **选中状态保持**：
     - **全量刷新路径**（调用 `Refresh()` 并重新设置 `ItemsSource`）：编辑前记录 `rowIndex = DataGrid.SelectedIndex` 和 `columnIndex = DataGrid.CurrentColumn.DisplayIndex`（或 `DataGrid.Columns.IndexOf(DataGrid.CurrentColumn)`）。刷新并重新设置 `ItemsSource` 后，执行 `DataGrid.SelectedItem = DataGrid.ItemsSource.Cast<object>().ElementAt(rowIndex)` 和 `DataGrid.CurrentColumn = DataGrid.Columns[columnIndex]` 恢复选中；若需进入编辑模式，再调用 `DataGrid.BeginEdit()`。全量刷新后 `Rows` 集合为全新分配的 `struct` 实例，禁止使用行对象引用恢复选中
     - **增量刷新路径**（调用 `ReplaceRow`）：因不替换 `ItemsSource`，可直接保留编辑前的 `SelectedItem` 和 `CurrentColumn` 引用恢复选中状态，无需重新定位
     - 两种路径均确保焦点不跳回表格顶部
...
变更反馈的具体形式：
- **全局未保存状态提示**：数据模型实现 `IAsyncSaveCapable` 时，控件层在信息栏区域显示未保存数量（如 "3 处未保存"）或存在性提示（"存在未保存变更"），数据源为 `IAsyncSaveCapable.UnsavedChangeCount` / `HasUnsavedChanges`。**控件层不维护逐单元格脏状态**，因此不提供单元格级橙色边框等细粒度脏标记视觉反馈；未保存提示为全局级别。若后端为即时持久化（无存盘按钮/无异步保存概念，未实现 `IAsyncSaveCapable`），此提示自动禁用，避免永久不触发的无效状态指示
- **状态栏提示（与全局未保存状态提示同源）**：`CalibrationEditorBase<T>` 的信息栏区域在存在未保存修改时显示未保存数量（如 "3 处未保存"），数据源为 `IAsyncSaveCapable.UnsavedChangeCount`。若后端只能提供布尔降级，则显示 "存在未保存变更"。启用条件与全局未保存状态提示一致
...
- **方向键（↑/↓/←/→）**：
  - 在**浏览模式**（非编辑模式）下：在单元格之间移动当前活动单元格（由 `SelectedItem` 和 `CurrentColumn` 共同决定）。`MapEditor` 和 `CubeEditor` 中由 DataGrid 内置导航处理；`CurveEditor` 中由 `ICurveTablePresenter` 实现方处理
...
- 用户使用方向键（↑/↓）或 Tab 在 `CubeEditor` 的平铺表格中导航时，DataGrid 的 `SelectedItem`/`CurrentColumn` 即时响应键盘输入而切换
...
- Tracker 读取新行的 `ZIndex`，若与当前激活 `ZIndex` 不同，则启动防抖计时（100–200ms）。**防抖期间表格的 `SelectedItem`/`CurrentColumn` 和行级高亮立即更新**，仅 3D 曲面图等待防抖结束后刷新
...
- 每次 `SelectedItem`/`CurrentColumn` 因键盘导航而变更时，`CubeEditor` 获取新 `SelectedItem` 所在行的 `CubeRowData`，调用 `OnSelectionChanged(row)`
...
当用户点击 z 列的任意单元格时，`CubeEditor` 调用 `ZSliceActivationTracker.OnZColumnClicked(row)`，Tracker 将该单元格所属行的 z 值设为激活切片（经过防抖），然后通过 `ActiveSliceChanged` 事件通知变更。**`CubeEditor` 在调用 `OnZColumnClicked` 之后，于 `ActiveSliceChanged` 事件处理器中（或直接在其后的同步代码中）自行处理"选中该切片内的第一个数据单元格"逻辑**——该行为不再是 Tracker 的职责。具体实现为：在事件处理器中计算该 z 切片的第一行 y 值对应的数据单元格坐标，程序化设置 `DataGrid.SelectedItem = targetRow` 和 `DataGrid.CurrentColumn = targetColumn`；若需进入编辑模式，再调用 `DataGrid.BeginEdit()`。

> **边界条件**：若 `XAxisLength == 0`（无 x 数据列），不存在数据单元格，跳过上述程序化选中步骤，仅更新 3D 曲面图和高亮样式。

键盘跨 z 切片导航时（如从当前 z 切片的最后一行 y 值按方向键移入下一 z 切片的第一行 y 值），表格的 `SelectedItem`/`CurrentColumn` 立即响应键盘输入而切换，`ZSliceActivationTracker` 内部引入短时间防抖（如 100–200ms），仅在用户停止快速导航后才通过 `ActiveSliceChanged` 事件真正刷新 3D 曲面图。此期间存在约 100–200ms 的视觉不一致（表格已切换行，3D 视图仍显示旧切片），在标定工具场景下属于可接受的轻微延迟；若需缓解，可在防抖期间为 3D 视图区域施加半透明遮罩（如 30% 透明度灰色覆盖），提示"正在加载"，防抖结束后移除遮罩并刷新图表。
...
2. **行级高亮**（轻微背景色，当前选中单元格所在的整行）— 由 DataGrid 的 `SelectedItem`（public）驱动，通过 `SelectionChanged` 事件或 `SelectedItemProperty` 变更实现；行级高亮样式通过 `DataGridRow` 的 `:selected` 伪类或基于 `SelectedItem` 匹配的样式选择器应用
...
- 行级高亮 → DataGrid `SelectedItem` 状态
- z 切片级高亮 → `ZSliceActivationTracker` 激活状态
- 行头文字样式变化 → DataGrid `SelectedItem` 状态（y 列）+ `ZSliceActivationTracker` 激活状态（z 列）
...
**理由**：全量 `Refresh()` + 重新设置 `ItemsSource` 虽然实现简单，但存在两个副作用：(1) 替换 `ItemsSource` 后 `SelectedItem`/`CurrentColumn` 被重置，焦点跳回表格顶部，破坏编辑连续性（需额外执行选中状态保持步骤）；(2) 对于 `CubeEditor`，每次编辑都执行 O(z×y) 的全量重建，大数据量时造成 UI 卡顿。
...
| 审查意见 | 修改措施 |
|---------|---------|
| **问题1**：CubeEditor z 列展示方案与需求存在偏差，未明确标注为需求折中。 | 在「设计决策2」开头增加显式声明区块，说明因 Avalonia DataGrid V12 原生不支持 `RowSpan` 式跨行单元格，"合并单元格"需求降级为"每行重复显示 + 视觉分组线"；补充未来升级路径（若后续 Avalonia 版本支持跨行合并或改用自定义表格组件，可重新评估）。 |
| **问题2**：`ICurveTablePresenter` 接口契约与职责描述不一致（列宽调整能力缺失）。 | 在接口中补充列宽调整相关契约：`double MinColumnWidth { get; set; }`、`event EventHandler<ColumnWidthChangedEventArgs>? ColumnWidthChanged`，并补充 `ColumnWidthChangedEventArgs` 事件参数类型定义（`ColumnIndex`、`NewWidth`）。 |
| **问题3**：`CalibrationEditorBase<T>` 模板部件名称未正式定义。 | 在「核心抽象」中新增「模板部件（Template Parts）」表格，正式定义 `PART_VariableSelector`（`ComboBox`）、`PART_InfoPanel`（`Panel`/`TextBlock`）、`PART_ContentHost`（`Panel`）三个部件的名称、预期类型、角色及缺失时的降级行为（前两者静默跳过，后者抛出异常）。 |
| **问题4**：`IMapData.GetValueMatrix()` / `ICubeData.GetSliceMatrix()` 返回数组的可变性语义未定义。 | 明确约定返回的 `double[,]` 为只读视图或深拷贝，调用方不应修改；若需修改数据，应通过数据模型写接口进行。后端实现方可选择返回内部数组的只读包装或执行深拷贝。 |
| **问题5**：编辑模式下方向键行为未定义。 | 在「键盘导航行为」中补充编辑模式方向键行为：方向键在 `TextBox` 内部移动光标（不退出编辑）；Tab 确认并移动到下一单元格；Enter 确认并保持焦点；Esc 取消编辑。 |
| **问题6**：空状态默认视觉呈现未定义。 | 补充 `CalibrationEditorBase<T>` 基类默认空状态行为：在主内容区中央渲染"无有效数据"占位文本，灰色调（`#808080`），水平垂直居中，字体大小与信息栏一致。子类可通过覆盖 `OnEnterEmptyState()` / `OnExitEmptyState()` 自定义。 |
| **问题7**：数值输入解析策略缺失。 | 在「单元格编辑与数据回写」中新增「数值输入解析策略」小节：统一使用 `CultureInfo.InvariantCulture` + `NumberStyles.Float` 解析；支持十进制和科学计数法；千分位和本地化小数点不纳入支持；`double` 溢出输入视为解析失败；编辑完成后按相同精度策略重新格式化。 |
| **问题8**：`IAsyncSaveCapable.SaveAsync()` 异常语义缺失。 | 明确保存失败时抛出异常（具体类型由后端实现决定），控件层 try/catch 捕获后在信息栏显示错误提示但不阻断编辑；明确 `UnsavedChangesChanged` 仅在 `HasUnsavedChanges` / `UnsavedChangeCount` 实际值变化时触发。 |
| **问题9**：键盘 Tab 导航在 CubeEditor 跨 z 切片边界时的行为未定义。 | 在「CubeEditor 跨 z 切片导航的特殊行为」中补充 Tab 跨边界行为：到达行末时自然延续到下一 z 切片第一行，与 DataGrid 默认行为一致。 |
| **问题10**：`ISurfaceChartPresenter` 缺少轴标签（名称+单位）设置契约。 | 在 `ISurfaceChartPresenter` 中补充 `void SetAxisLabels(string xName, string xUnit, string yName, string yUnit, string valueName, string valueUnit)` 方法，在 `LoadMapData`/`LoadSliceData` 之后调用以标注轴信息。 |
| **问题11**：文档版本号与文件名不一致。 | 将文档标题统一为 v7，文件名对应为 `a_v5_design_v1.md`；概述中列出相对 v6 的 13 项核心变化。 |
| **问题12**：`ActiveZSliceChangedEventArgs` 缺少 z 轴实际值。 | 在事件参数中补充 `double OldZValue { get; }` 和 `double NewZValue { get; }`，由 `ICubeData.ZValues` 按索引提取。 |
| **问题13**：`ZSliceActivationTracker` 防抖延迟未参数化。 | 构造函数增加可选的 `TimeSpan debounceDelay` 参数，默认值为 `TimeSpan.FromMilliseconds(150)`；暴露 `DebounceDelay` 只读属性。 |
| **问题14**：精度展示策略编辑模式后备格式未细化。 | 在「精度展示策略」中补充后备格式规则：`FormatString` 为空且 `DisplayPrecision` 为 0 时默认使用 `G6`；明确编辑完成后重新格式化的精确规则（刷新后的显示值按相同优先级策略重新格式化）。 |
...
| 审查意见 | 修改措施 |
|---------|---------|
| **严重-问题1**：Avalonia DataGrid 不存在 `CurrentCell` 属性，文档多处引用该不存在的属性，按字面编码将导致编译失败。 | 将文档中所有 `DataGrid.CurrentCell` 引用统一替换为 Avalonia 实际 API 组合：`SelectedItem`（public，可读写）+ `CurrentColumn`（public，可读写）+ `DisplayIndex` + `BeginEdit()`。覆盖编辑回写、键盘导航、z 切片激活联动等全部关键路径。 |
| **严重-问题2**：单元格级脏标记（橙色边框）与 `IAsyncSaveCapable` 接口能力矛盾——接口仅提供全局布尔/总数，无逐单元格脏状态查询能力。 | 采用方案 A（降级为全局提示）：移除「单元格级脏标记」承诺，明确声明"控件层不维护逐单元格脏状态，仅显示全局未保存提示"。`IAsyncSaveCapable` 的 `HasUnsavedChanges` / `UnsavedChangeCount` 仅支撑信息栏全局提示。 |
| **严重-问题3**：`ActiveZSliceChangedEventArgs.OldZValue` 在 `OldZIndex == -1` 时存在 `ICubeData.ZValues[-1]` 数组越界风险；`NewZValue` 同理。 | 规定 `OldZIndex`/`NewZIndex < 0` 时对应 `OldZValue`/`NewZValue` 返回 `double.NaN`；`ActiveSliceChanged` 事件仅在 `OldZIndex >= 0` 且 `NewZIndex >= 0` 时触发，杜绝无效索引导致的运行时异常。 |
| **一般-问题4**：`ILineChartPresenter` 缺少折线图轴标签的显式设置契约，与 `ISurfaceChartPresenter` 不对称。 | 在 `ILineChartPresenter` 中补充 `void SetAxisLabels(string xName, string xUnit, string zName, string zUnit)` 方法，与 `ISurfaceChartPresenter` 保持对称；在 `CurveEditor` 变量切换流程中明确加载数据后调用 `SetAxisLabels`。 |
| **一般-问题5**：全量刷新后的选中状态恢复策略存在歧义，行对象引用在 `ItemsSource` 替换后失效。 | 明确区分两种刷新路径：全量刷新（`Refresh()` + 重新设置 `ItemsSource`）后只能使用「行索引 + 列索引」恢复，禁止使用行对象引用；增量刷新（`ReplaceRow`）下因不替换 `ItemsSource`，可保留原有行对象引用恢复。 |
| **一般-问题6**：`CubeEditor` 平铺表格的排序责任未在契约层明确。 | 在 `ITableDataSource<TRow>` 职责描述中增加排序契约：`CubeRowData` 按 `ZIndex` 升序分组、组内按 `YIndex` 升序排列；`MapRowData` 按 `YIndex` 升序排列。排序是适配器的强制职责。 |
| **一般-问题7**：`CubeRowData`/`MapRowData` 行头值（`ZValue`/`YValue`）的类型未定义，导致格式化职责归属不清。 | 明确 `ZValue` 和 `YValue` 为 `string` 类型，由 `CubeTableAdapter`/`MapTableAdapter` 在生成行数据时按 `FormatString` → `DisplayPrecision` → `G6` 策略预先格式化；行头列 `CellTemplate` 直接绑定纯文本展示。 |
| **一般-问题8**：动态列生成时 `FormatString`/`DisplayPrecision` 向 `DataGridTextColumn.Binding.StringFormat` 的传递机制缺失。 | 在 `ITableDataSource<TRow>` 中新增 `string? FormatString { get; }` 和 `int DisplayPrecision { get; }` 属性，由适配器从 `ICalibrationData` 透传；供 Editor 动态生成数据列时设置 `Binding.StringFormat`。 |
| **轻微-问题9**：行级高亮驱动源与 Avalonia DataGrid 实际 API 不匹配，`CurrentItem` 不可外部访问。 | 将行级高亮驱动源从 `DataGrid.CurrentItem`（protected）修正为 `DataGrid.SelectedItem`（public），通过 `SelectionChanged` 事件或 `SelectedItemProperty` 变更实现。 |
| **Judge 补充-问题10**：`CubeEditor` z 列点击后「选中第一个数据单元格」未考虑 `XAxisLength == 0` 边界情况。 | 在「z 切片激活与 3D 视图联动」中补充边界条件：若 `XAxisLength == 0`，不存在数据单元格，跳过"选中第一个数据单元格"步骤，仅更新 3D 曲面图和高亮样式。 |
| **Judge 补充-问题11**：图表呈现器生命周期管理缺失，`Detach()` 调用时机未约定。 | 在 `CurveEditor`/`MapEditor`/`CubeEditor` 职责中明确 `Detach()` 调用时机：变量切换导致旧图表丢弃时、控件从视觉树卸载时。在「变量切换流程」中补充旧呈现器 `Detach()` 步骤。 |
