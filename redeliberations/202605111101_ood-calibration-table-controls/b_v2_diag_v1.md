...
### 问题 1：`DataGridRow` 行高限制策略存在 Avalonia 样式系统事实错误

**问题描述**：设计文档在「列宽/行高调整策略」中建议使用 `DataGrid.Styles` 为 `DataGridRow` 设置 `MinHeight` 和 `MaxHeight`：
...
但 `MinHeight` 和 `MaxHeight` 是 `Layoutable` 的 `DirectProperty`，而 Avalonia 官方文档明确指出：**`DirectProperty` 和 CLR 属性 cannot be styled and therefore do not participate in this precedence**。上述样式语法在 Avalonia 中不会生效。

**所在位置**：「关键行为契约」→「列宽/行高调整策略」→「MapEditor / CubeEditor（DataGrid）」小节

**严重程度**：严重

**改进建议**：
- 方案 A：改用 `DataGrid.RowHeight` 设置固定行高（若最小/最大约束非硬性需求）。
- 方案 B：在 `LoadingRow` 事件处理器中以代码方式逐行设置 `e.Row.MinHeight = 24` 和 `e.Row.MaxHeight = 48`。
- 方案 C：定义一个继承自 `DataGridRow` 的自定义行类，在 `ControlTemplate` 中通过模板绑定或嵌套布局容器实现高度约束。

---

### 问题 2：`readonly struct` 与 DataGrid 标准编辑机制不兼容的根因分析错误
...
### 问题 3：单元格编辑后的全量刷新策略存在选中状态丢失和性能风险，但未给出应对方案
...
**改进建议**：
- 在编辑流程中补充「选中状态保持」步骤：编辑前保存 `CurrentCell` 的坐标（行索引 + 列索引），`Refresh()` 后根据新 `ItemsSource` 重新定位并恢复选中。
- 评估增量刷新可行性：若将 `ITableDataSource<TRow>.Rows` 改为 `ObservableCollection<TRow>`，可通过 `Rows[index] = newRow` 实现单元素替换，DataGrid 仅刷新对应行而无需全量重建。
- 若坚持全量刷新，应在「并发设计」或「设计决策」中明确说明性能兜底方案（如异步刷新、延迟刷新等）。

---

## 中等问题

### 问题 4：数据模型元素级变更的 `PropertyChanged` 通知约定缺失
...
### 问题 5：`ISurfaceChartPresenter` 两个加载方法的参数风格不一致
...
### 问题 6：`ZSliceActivationTracker` 核心事件签名未定义
...
### 问题 7：`ICurveTablePresenter` 缺少程序化选中设置能力
...
### 问题 8：脏标记跟踪的 `IAsyncSaveCapable` 接口未定义但被引用
...
### 问题 9：z/y 行头文字在激活状态下的加粗/颜色变化需求未响应
...
### 问题 10：`CubeEditor` 初始默认 z 切片语义未定义
...
### 问题 11：`ITableDataSource.GetXIndexFromColumn` 的列索引语义不明确
...
### 问题 12：键盘跨 z 切片导航时表格与 3D 视图的视觉不一致未处理
...
但在**落地实现层面**仍存在若干缺口：Avalonia 样式系统的技术事实错误（`DirectProperty` 不可样式化）、值类型编辑不兼容的根因误述、全量刷新策略的副作用未处理，以及多个接口/类的成员签名不完整（`ZSliceActivationTracker` 事件、`IAsyncSaveCapable`、`ICurveTablePresenter` 选中输入）。这些问题若不在实现前修正，将导致实现者产生困惑或做出错误的技术决策。

建议下一轮迭代优先处理**严重问题 1–3**（技术事实错误和全量刷新策略），其次处理**中等问题 4–9**（接口契约完整性和需求响应遗漏）。
