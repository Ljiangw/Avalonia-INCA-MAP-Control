...
1. **修正 `readonly struct` 与 DataGrid 编辑机制不兼容的根因分析**：从「不可变性导致无法写回」修正为「值类型的 boxing 副本问题导致无论 `readonly struct` 还是可变 `struct` 都无法通过 DataGrid 标准编辑提交直接修改原始实例」，并补充 `class` 降级方案的完整权衡分析。
2. **补充单元格编辑后的选中状态保持机制**：在 `MapEditor`/`CubeEditor` 的编辑流程中增加编辑前保存 `CurrentCell` 坐标、`Refresh()` 后恢复选中的步骤，解决全量刷新导致焦点跳回顶部的问题。
3. **补充增量刷新可行性评估**：提出将 `ITableDataSource<TRow>.Rows` 改为 `ObservableCollection<TRow>` 实现单元素替换的优化方向。
4. **统一 `ISurfaceChartPresenter` 参数风格**：`LoadMapData` 改为接收原始数组，与 `LoadSliceData` 风格一致。
5. **补充遗漏的接口定义和事件签名**：为 `ZSliceActivationTracker` 定义 `ActiveSliceChanged` 事件和 `ActiveZSliceChangedEventArgs`；为 `ICurveTablePresenter` 补充程序化选中方法；在「核心抽象」中正式定义 `IAsyncSaveCapable`。
6. **响应需求遗漏**：补充 z/y 行头文字在激活状态下的加粗/变色样式变化；明确默认 z 切片语义、`GetXIndexFromColumn` 列索引语义、3D 视图切换动画决策、键盘跨 z 切片导航的视觉不一致处理策略。
7. **明确 `ICalibrationData` 元素级变更通知约定**：规定元素级写方法触发 `PropertyChanged` 时使用 `string.Empty` 作为属性名。
...
**元素级变更通知约定**：
元素级写方法（如 `SetXValue`、`SetValue` 等）在修改数据后应触发 `PropertyChanged` 事件，属性名约定为 `string.Empty`。在 Avalonia 绑定系统中，`string.Empty` 表示所有绑定属性均可能变更，触发全局刷新。控件层收到该通知后，调用 `ITableDataSource<TRow>.Refresh()` 或 `ICurveTablePresenter.Refresh()` 刷新显示。若后端需要区分「仅数据值变更」与「元信息变更」，可额外定义专门的 `ValuesChanged` 事件，控件层订阅该事件执行数据级别的局部刷新。
...
- `void SetXValue(int index, double value)` — 更新指定索引处的 x 值；更新完成后触发 `PropertyChanged`，属性名为 `string.Empty`
- `void SetZValue(int index, double value)` — 更新指定索引处的 z 值；更新完成后触发 `PropertyChanged`，属性名为 `string.Empty`

`IMapData`：
- `int XAxisLength { get; }`、`int YAxisLength { get; }` — 各轴维度长度
- `double GetValue(int xIndex, int yIndex)` — 读取二维矩阵指定位置的 z 值
- `void SetValue(int xIndex, int yIndex, double value)` — 更新二维矩阵指定位置的 z 值；更新完成后触发 `PropertyChanged`，属性名为 `string.Empty`
...
- `void SetValue(int xIndex, int yIndex, int zIndex, double value)` — 更新三维张量指定位置的值；更新完成后触发 `PropertyChanged`，属性名为 `string.Empty`
- `IReadOnlyList<double> XValues { get; }` — x 轴坐标值序列
- `IReadOnlyList<double> YValues { get; }` — y 轴坐标值序列
...
**角色**：标记后端数据模型是否支持异步保存机制。

**职责**：作为可选能力标记接口，供控件层检测以决定是否启用脏标记视觉反馈和状态栏未保存提示。可扩展为包含 `bool HasUnsavedChanges { get; }` 属性和 `Task SaveAsync()` 方法的完整契约，但最小形态为纯标记接口。

**协作**：由后端数据模型选择性实现；`CalibrationEditorBase<T>` 在初始化或变量切换时检测 `SelectedVariable` 是否实现此接口，若实现则启用脏标记跟踪逻辑和状态栏未保存提示。若后端为即时持久化（无存盘按钮/无异步保存概念），不实现此接口，脏标记相关反馈自动禁用。

**类型形态**：接口。仅作为能力标记，不强制所有后端模型实现。
...
- 脏标记检测：检测 `SelectedVariable` 是否实现 `IAsyncSaveCapable`，决定是否启用脏标记视觉反馈和状态栏未保存提示
...
- 支持程序化选中控制：变量切换后重置选中，或图表联动时反向选中表格单元格
...
- `void SelectCell(int columnIndex, CurveTableRow row)` — 程序化设置选中单元格；`CurveTableRow` 为枚举（`X` / `Z`）
...
**关键成员**：
- `IReadOnlyList<TRow> Rows { get; }` — 纵向行集合，可直接绑定到 `DataGrid.ItemsSource`。在增量刷新方案中，此属性可改为 `ObservableCollection<TRow>`，通过单元素替换（`Rows[index] = newRow`）实现 DataGrid 局部刷新，避免全量重建 `ItemsSource` 导致的选中状态丢失
- `int GetXIndexFromColumn(int columnIndex)` — 将 DataGrid 的原始列索引（包含所有行头列，如 z 列、y 列）映射为原始数据的 x 轴索引。由 `ITableDataSource` 实现方内部处理行头列的偏移映射
...
- `void LoadMapData(double[,] mapData, IReadOnlyList<double> xValues, IReadOnlyList<double> yValues)` — 加载二维 Map 的原始矩阵数据（MapEditor 使用）
...
**协作**：`CurveEditor` 消费 `ILineChartPresenter`；`MapEditor` 消费 `ISurfaceChartPresenter`（加载完整 Map 原始数组）；`CubeEditor` 消费 `ISurfaceChartPresenter`（加载当前激活 z 切片的二维矩阵）。
...
**职责**：
- 维护当前激活的 z 值索引；初始化时默认激活第一个 z 切片（`ZIndex = 0`），后续可在实现阶段根据用户记忆偏好调整
...
**关键事件**：
- `event EventHandler<ActiveZSliceChangedEventArgs> ActiveSliceChanged` — 激活 z 切片变更通知事件。事件参数 `ActiveZSliceChangedEventArgs` 包含 `int NewZIndex`（新的激活 z 索引）和 `int OldZIndex`（旧的激活 z 索引）。事件在防抖结束后触发，即用户停止快速导航且防抖延迟（如 100–200ms）届满后才真正发出，避免中间过渡状态的无效通知。
...
用户触发单元格进入编辑模式（双击）→ 单元格展示编辑模板（如 `TextBox`）→ 用户输入数值 → 输入完成时（Enter/失去焦点），Editor 拦截 `CellEditEnding` 事件，从对应类型的行数据对象中提取坐标索引属性，调用原始数据模型的写方法更新对应位置 → 模型触发 `INotifyPropertyChanged.PropertyChanged`（属性名为 `string.Empty`）→ Editor 订阅到该通知后调用 `ITableDataSource<TRow>.Refresh()` 重新生成行集合，并重新设置 `DataGrid.ItemsSource`，完成表格刷新；若该单元格值同时影响图表，图表呈现器接收通知后重绘。

**选中状态保持**：为避免 `ItemsSource` 替换后 `CurrentItem`/`CurrentCell`/`SelectedItem` 被重置导致焦点跳回表格顶部，编辑流程中补充以下步骤：
1. 编辑前保存当前 `CurrentCell` 坐标（行索引 + 列索引）
2. 调用 `Refresh()` 重新生成行集合
3. 重新设置 `DataGrid.ItemsSource`
4. 根据保存的坐标重新定位并恢复 `CurrentCell` 和 `SelectedItem`

**增量刷新优化（可选）**：若将 `ITableDataSource<TRow>.Rows` 改为 `ObservableCollection<TRow>`，可通过 `Rows[index] = newRow` 实现单元素替换，DataGrid 仅刷新对应行而无需全量重建 `ItemsSource`，从根本上避免选中状态丢失问题。此方案适用于编辑频率高但数据量中等的场景；若行数极端大（>10,000），`ObservableCollection` 的变更通知开销需额外评估。
...
变更反馈的具体形式：
- **脏标记视觉反馈**：已编辑但尚未持久化的单元格以边框变色（如橙色细边框）提示已修改。此反馈仅在后端实现异步保存机制时生效；控件层通过检测后端是否实现 `IAsyncSaveCapable` 标记接口来决定是否启用脏标记跟踪。若后端为即时持久化（无存盘按钮/无异步保存概念），此反馈自动禁用，避免永久不触发的无效状态指示
- **状态栏提示**：`CalibrationEditorBase<T>` 的信息栏区域在存在未保存修改时显示未保存数量（如 "3 处未保存"），启用条件与脏标记视觉反馈一致
- **模型变更通知**：数据模型写接口触发 `PropertyChanged` 后，Editor 调用 `ITableDataSource<TRow>.Refresh()`（MapEditor/CubeEditor）或 `ICurveTablePresenter.Refresh()`（CurveEditor）刷新单元格显示，作为最基础的变更确认
...
**视觉不一致处理**：键盘跨 z 切片导航时，表格当前行即时切换，但 3D 曲面图因防抖延迟刷新，存在约 100–200ms 的视觉不一致。在标定工具场景下，此短暂不一致为可接受的权衡，以换取防抖带来的性能收益。若需缓解，可在防抖期间为 3D 视图区域添加轻量过渡效果（如半透明遮罩覆盖旧数据，或先清空视图等待新数据加载）。
...
4. **行头文字样式变化**（z 列/y 列文字加粗或变色）— 当前行被选中时 y 列文字加粗或变色；当前 z 切片激活时 z 列文字加粗或变色。通过 `DataGridCell` 样式选择器或 `LoadingRow` 事件中对行头单元格附加样式类（如 `active-row-header` / `active-z-header`）实现
...
在 `MapEditor`/`CubeEditor` 中，由于行数据类型为 `readonly struct`（以及更根本的原因——值类型的 boxing 副本问题），DataGrid 的标准编辑双向绑定自动提交机制无法直接写回原始实例。根本原因在于：DataGrid 将行数据赋给 `DataGridRow.DataContext`（`object` 类型属性）时发生 boxing，后续标准编辑提交尝试对 boxing 副本进行写操作，该修改无法反映回 `Rows` 集合中的原始实例。`readonly` 在此基础上额外禁止了 setter 调用，但即使降级为可变 `struct`，boxing 副本问题仍然存在。只有将行数据降级为 `class`（引用类型），DataGrid 才能通过引用直接修改 `Rows` 集合中的原始对象，从而利用内置编辑提交机制。

设计采用 **完全手动事件拦截** 策略：
- 在 `CellEditEnding` 事件中拦截编辑值，手动解析并验证
- 解析失败时，通过设置编辑模板内 `TextBox` 的自定义验证状态（如附加 `DataValidationErrors` 属性或边框样式变更）实现红色边框反馈
- 验证通过后，调用原始数据模型写接口，然后取消 `CellEditEnding` 事件的默认提交（`e.Cancel = true`），由 Editor 监听模型的 `PropertyChanged` 后主动调用 `ITableDataSource<TRow>.Refresh()` 并重新设置 `DataGrid.ItemsSource`，完成 UI 刷新
...
- **全量刷新性能兜底**：若 `CubeEditor` 数据量极端大（如 z 数 × y 数 > 10,000），每次编辑后的全量 `Refresh()` + `ItemsSource` 重建可能导致 UI 卡顿。性能兜底方案包括：
  - 采用 `ObservableCollection<TRow>` 增量刷新，单元素替换避免全量重建
  - 引入异步刷新：编辑完成后先立即更新当前单元格显示，在后台线程准备新行集合，完成后切换 `ItemsSource`
  - 引入延迟刷新：连续编辑时批量合并刷新请求，减少刷新频率
  - 引入编辑后批量刷新：用户完成一段连续编辑（如失去表格焦点或按下特定快捷键）后统一刷新
...
`ISurfaceChartPresenter` 的两个加载方法统一为接收原始数组的风格（`double[,]` + `IReadOnlyList<double>` 坐标值），由调用方（`MapEditor`/`CubeEditor`）负责从数据模型提取数据。此设计保持图表抽象层与数据契约层解耦，图表实现方无需引用 `IMapData`/`ICubeData` 接口。

`CubeEditor` 切换激活 z 切片时，3D 曲面图采用**直接瞬时刷新**（与表格即时响应保持一致），不引入过渡动画。过渡动画作为可选增强在阶段 2 评估，若后续用户体验测试表明需要更平滑的视觉连贯性，可引入轻量淡入淡出过渡。
...
但 `readonly struct`（以及任何 `struct`）与 DataGrid 标准单元格编辑的双向绑定自动提交机制不兼容的根本原因是**值类型的 boxing 副本问题**：DataGrid 将行数据赋给 `DataGridRow.DataContext`（`object` 类型属性）时发生 boxing，后续标准编辑提交尝试对 boxing 副本进行写操作，该修改无法反映回 `Rows` 集合中的原始实例。`readonly` 在此基础上额外禁止了 setter 调用，但即使降级为可变 `struct`，boxing 副本问题仍然存在，标准编辑提交仍无法生效。只有将行数据降级为 `class`（引用类型），DataGrid 才能通过引用直接修改 `Rows` 集合中的原始对象，从而利用内置编辑提交机制。

设计采用 **完全手动事件拦截** 策略应对此限制：在 `CellEditEnding` 事件中拦截编辑值，手动解析验证后直接调用后端模型写接口，然后取消默认提交（`e.Cancel = true`），由模型 `PropertyChanged` 通知触发 UI 刷新。
...
`class` 降级方案的补充权衡：
- **收益**：恢复 DataGrid 内置编辑提交和验证模板，减少手动事件拦截的维护成本
- **代价**：每行产生堆分配，大数据量时 GC 压力显著增加；失去坐标索引的内联存储优势，属性访问需通过引用间接寻址
...
| 审查意见 | 修改措施 |
|---------|---------|
| **v4 审查-问题1**：Avalonia DataGrid 不存在 `UnloadingRow` 事件，设计方案依赖该事件清理行回收前的样式类残留，会导致虚拟化行回收后残留状态被错误应用到后续重用的行上。 | 删除「z 切片级高亮状态与 DataGrid 虚拟化联动」中对 `UnloadingRow` 的所有引用。改为「`LoadingRow` 先无条件清理再判断」策略：在 `LoadingRow` 事件处理器中先移除该行已附加的 z 切片高亮样式类（避免重用车池中的残留状态），再根据 `DataContext` 中的 `ZIndex` 判断是否重新附加。同时补充 `DataGridRow.DataContextChanged` 事件作为后备清理机制，确保行重用时视觉状态始终与当前数据一致。 |
| **v4 审查-问题2**：`readonly struct` 行数据不支持 `INotifyPropertyChanged`，绑定系统无法自动感知原始数据模型的值变更，导致非编辑路径下的数据变更（如后端标定数据从 ECU 更新后）无法正确反映到表格 UI。 | 在 `ITableDataSource<TRow>` 接口中补充 `void Refresh()` 方法，职责为响应外部原始数据模型的变更通知并重新生成 `Rows` 行集合。在「关键行为契约」→「单元格编辑与数据回写」中明确：编辑完成后调用模型写接口，模型触发 `PropertyChanged` 后，Editor 订阅到该通知并调用 `ITableDataSource<TRow>.Refresh()` 重新生成行集合，再重新设置 `DataGrid.ItemsSource` 完成刷新。此路径同时适用于编辑路径和非编辑路径下的外部数据变更。 |
| **v4 审查-附带**：`ISurfaceChartPresenter` 声明两个参数类型差异的 `LoadData` 重载，调用方意图表达不够显式。 | 将两个重载拆分为语义明确的独立方法名：`LoadMapData(IMapData data)`（MapEditor 使用）和 `LoadSliceData(double[,] sliceData, IReadOnlyList<double> xValues, IReadOnlyList<double> yValues)`（CubeEditor 使用）。同步更新「核心抽象」中 `ISurfaceChartPresenter` 的关键成员描述和「设计决策 5」中的相关引用。 |
| **v4 审查-附带**：脏标记视觉反馈（橙色边框）和状态栏未保存提示的设计假设后端存在异步保存机制，若后端为即时持久化则反馈将永久不触发。 | 在「变更反馈机制」中明确：脏标记视觉反馈和状态栏未保存提示的启用条件是后端实现 `IAsyncSaveCapable` 标记接口；控件层通过检测该接口决定是否启用脏标记跟踪。若后端为即时持久化（无存盘按钮/无异步保存概念），此反馈自动禁用，避免实现阶段困惑。 |
| **v4 审查-附带**：`readonly struct` 行数据的 GC 收益表述过于绝对，忽略了 `DataGrid.DataContext` 赋值时的 boxing。 | 修正 `MapRowData` / `CubeRowData` 的「类型形态」描述：明确 `DataGrid` 将行数据赋给 `DataGridRow.DataContext`（`object` 类型属性）时仍会发生一次由 Avalonia 框架决定的 boxing，无法避免；但行数据本身避免了堆分配和接口引用的额外装箱，总体 GC 压力仍显著低于 `class` 方案。 |
| **v4 审查-附带**：`ZSliceActivationTracker` 的防抖策略未明确事件源线程与调度假设之间的关系。 | 在 `ZSliceActivationTracker` 的「职责」描述中补充线程假设说明：Tracker 的事件源为 `CubeEditor` 在 UI 线程上订阅的 DataGrid 选中变更事件，因此防抖触发起点在 UI 线程；`Task.Delay` 的延续线程由线程池调度，但公开事件由订阅方（`CubeEditor`）通过 `Dispatcher.UIThread.Post` 将状态变更调度回 UI 线程。Tracker 自身不直接操作 Avalonia 视觉类型，保持可测试性。 |

## 修订说明（v5）

| 审查意见 | 修改措施 |
|---------|---------|
| **严重-问题1**：`readonly struct` 与 DataGrid 标准编辑机制不兼容的根因分析错误。设计文档将原因归因于「不可变性」，实际根本原因是值类型的 boxing 副本问题。 | 在「错误处理策略」→「输入验证错误」中修正根因说明：明确指出值类型的 boxing 副本问题是根本原因——DataGrid 将行数据赋给 `DataGridRow.DataContext`（`object` 类型）时发生 boxing，标准编辑提交对 boxing 副本的修改无法反映回 `Rows` 集合中的原始实例。`readonly` 仅在此基础上额外禁止了 setter 调用。明确说明无论 `readonly struct` 还是可变 `struct` 都必须采用手动事件拦截，只有降级为 `class` 才能利用 DataGrid 内置编辑提交。在设计决策 9 中补充 `class` 降级方案的 GC 影响和坐标索引内联存储损失，使权衡分析完整。 |
| **严重-问题2**：单元格编辑后的全量刷新策略存在选中状态丢失和性能风险，但未给出应对方案。 | 在「单元格编辑与数据回写」→「MapEditor / CubeEditor」中补充「选中状态保持」步骤：编辑前保存 `CurrentCell` 坐标（行索引 + 列索引），`Refresh()` 后根据新 `ItemsSource` 重新定位并恢复选中。补充增量刷新可行性评估：若将 `ITableDataSource<TRow>.Rows` 改为 `ObservableCollection<TRow>`，可通过 `Rows[index] = newRow` 实现单元素替换，DataGrid 仅刷新对应行。在「并发设计」中补充性能兜底方案：异步刷新、延迟刷新、编辑后批量刷新等策略。 |
| **中等-问题3**：数据模型元素级变更的 `PropertyChanged` 通知约定缺失。 | 在 `ICalibrationData`「核心抽象」中新增「元素级变更通知约定」小节：明确元素级写方法触发 `PropertyChanged` 事件时属性名约定为 `string.Empty`（表示所有属性均可能变更）；说明控件层收到通知后调用 `Refresh()` 的刷新策略。提及后端可额外定义 `ValuesChanged` 事件以区分数据值变更与元信息变更。 |
| **中等-问题4**：`ISurfaceChartPresenter` 两个加载方法的参数风格不一致。 | 将 `LoadMapData(IMapData data)` 改为 `LoadMapData(double[,] mapData, IReadOnlyList<double> xValues, IReadOnlyList<double> yValues)`，与 `LoadSliceData` 统一为接收原始数组的风格。在设计决策 5 中补充说明：由调用方负责从数据模型提取数据，保持图表抽象层与数据契约层解耦。 |
| **中等-问题5**：`ZSliceActivationTracker` 核心事件签名未定义。 | 在 `ZSliceActivationTracker`「核心抽象」中补充 `ActiveSliceChanged` 事件定义：`event EventHandler<ActiveZSliceChangedEventArgs> ActiveSliceChanged`，参数包含 `NewZIndex` 和 `OldZIndex`。明确事件在防抖结束后触发，避免中间过渡状态的无效通知。 |
| **中等-问题6**：`ICurveTablePresenter` 缺少程序化选中设置能力。 | 在 `ICurveTablePresenter`「关键成员」中补充 `void SelectCell(int columnIndex, CurveTableRow row)` 和 `void ClearSelection()`；定义 `CurveTableRow` 为枚举（`X` / `Z`）。明确用于变量切换后重置选中或图表联动时反向选中表格单元格。 |
| **中等-问题7**：脏标记跟踪的 `IAsyncSaveCapable` 接口未定义但被引用。 | 在「核心抽象」中新增 `IAsyncSaveCapable` 接口的完整定义：作为可选能力标记接口，供后端数据模型选择性实现；`CalibrationEditorBase<T>` 检测 `SelectedVariable` 是否实现此接口以决定是否启用脏标记跟踪。说明最小形态可为纯标记接口，也可扩展为含 `HasUnsavedChanges`/`SaveAsync` 的完整契约。 |
| **中等-问题8**：z/y 行头文字在激活状态下的加粗/颜色变化需求未响应。 | 在「高亮叠加策略」中补充第四层视觉反馈：行头文字样式变化（z 列/y 列文字加粗或变色）。明确当前行被选中时 y 列文字加粗或变色，当前 z 切片激活时 z 列文字加粗或变色。实现方式通过 `DataGridCell` 样式选择器或 `LoadingRow` 事件中对行头单元格附加样式类（如 `active-row-header` / `active-z-header`）。 |
| **轻微-问题9**：`CubeEditor` 初始默认 z 切片语义未定义。 | 在「变量切换流程」中明确 CubeEditor 变量切换时「重置 `ZSliceActivationTracker` 为默认 z 切片（`ZIndex = 0`，即第一个 z 切片）」。在 `ZSliceActivationTracker`「职责」中补充：初始化时默认激活第一个 z 切片，后续可在实现阶段根据用户记忆偏好调整。 |
| **轻微-问题10**：`ITableDataSource.GetXIndexFromColumn` 的列索引语义不明确。 | 在 `ITableDataSource<TRow>`「关键成员」中明确：`columnIndex` 为 DataGrid 的原始列索引（包含所有行头列，如 z 列、y 列），由实现方内部处理行头列的偏移映射。 |
| **轻微-问题11**：键盘跨 z 切片导航时表格与 3D 视图的视觉不一致未处理。 | 在「z 切片激活与 3D 视图联动」中补充视觉不一致处理策略：明确约 100–200ms 的不一致在标定工具场景下为可接受的权衡；若需缓解，可在防抖期间为 3D 视图区域添加轻量过渡效果（如半透明遮罩或先清空视图等待加载）。 |
| **轻微-问题12**：需求"推断与待确认事项"中 3D 曲面图数据源切换动画未给出决策结论。 | 在「设计决策 5」中补充明确决策：`CubeEditor` 切换激活 z 切片时，3D 曲面图采用直接瞬时刷新（与表格即时响应保持一致），不引入过渡动画。过渡动画作为可选增强在阶段 2 评估。 |
